...
//            $r = &$this->file->nextRecord();
	...
	function createDate($numValue)
    {
         ...
         if ($numValue > 1) {
			...
			$raw = $utcValue;
			//New format for dates yyyy-mm-dd to use in mysql inserts
			$tmp_timestamp=((real)$numValue-(real)25568)*(real)(24*60*60);
			$string=date("Y-m-d",$tmp_timestamp-86400);
			$raw = $tmp_timestamp;
         } else {
			$raw = $numValue;
...
/*
         if ($numValue > 1) {
			...
			//New format for dates yyyy-mm-dd to use in mysql inserts
			$tmp_timestamp=((real)$numValue-(real)25568)*(real)(24*60*60);
			$string=date("Y-m-d",$tmp_timestamp);
			$raw = $tmp_timestamp;
         } else {
			$raw = $numValue;
			$hours = floor($numValue * 24);
