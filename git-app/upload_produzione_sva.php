...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
        	<h1 class="page-header">Upload Excel: result</h1>
        </div><!-- /.col-lg-12 -->
<?php
...
echo '<br>Inserisco i dati sopra nella tabella  "'.MESE_CORRENTE.'"  al momento arbitrariamente creata. (TRUNCATE POI INSERT)<br><br>';
...
mysql_select_db(DBNAME);
	...
	$query_trunc="TRUNCATE table ".MESE_CORRENTE;
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++)
	...
	$VENDITORE=			strtolower("".htmlentities($data->sheets[0]['cells'][$i][14], ENT_QUOTES)."");
	$ORIGINE= 			strtolower("".htmlentities($data->sheets[0]['cells'][$i][15], ENT_QUOTES)."");
	$STAND= 			strtolower("".htmlentities($data->sheets[0]['cells'][$i][16], ENT_QUOTES)."");
	$OPERATORE_CC= 		strtolower("".htmlentities($data->sheets[0]['cells'][$i][17], ENT_QUOTES)."");
	$CODICE_INSERIMENTO=strtolower("".htmlentities($data->sheets[0]['cells'][$i][18], ENT_QUOTES)."");
	$DETTAGLI= 			strtolower("".htmlentities($data->sheets[0]['cells'][$i][19], ENT_QUOTES)."");
	$PACCHETTO_TECNOLOGIA= $PACCHETTO."_".$TECNOLOGIA;
	$NOTE= 				strtolower("".htmlentities($data->sheets[0]['cells'][$i][20], ENT_QUOTES)."");
	$NOTE2= 			strtolower("".htmlentities($data->sheets[0]['cells'][$i][21], ENT_QUOTES)."");
	...
	$query="insert into  ".MESE_CORRENTE." 
	(
	DATA,
	SEGMENTO,
	RAG_SOC,
	...
	$query_trunc="DROP table  ".MESE_CORRENTE_STAND ;
	$trunc=mysql_query($query_trunc);
	if($trunc)echo"<br><br> ".MESE_CORRENTE_STAND."  TRONCATA<br><br>";

	$query_duplica="CREATE TABLE  ".MESE_CORRENTE_STAND."  AS SELECT DATA, CODICE_INSERIMENTO, STAND, PACCHETTO, TECNOLOGIA, PACCHETTO_TECNOLOGIA, GETTONE, PROVVIGIONE, NETTO FROM  ".MESE_CORRENTE." ;";
	$duplica=mysql_query($query_duplica);
	if($duplica)echo"<br><br> ".MESE_CORRENTE_STAND."  TABELLA COPIATA<br><br>";
	
	$query_trunc="DELETE FROM ` ".MESE_CORRENTE_STAND." ` WHERE `CODICE_INSERIMENTO` = \"\";";
	$trunc=mysql_query($query_trunc);
	if($trunc)echo"<br><br> ".MESE_CORRENTE."  ELIMINATI CODICI INSERIMENTO VUOTI<br><br>";
